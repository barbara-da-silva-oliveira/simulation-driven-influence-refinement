function row = makeSummaryRow(meta, srp, evalInfo, runDir, logs_file, json_file, status, errMsg)
    ...
    row.campaign_id = string(meta.campaign_id);
    row.scenario_id = string(meta.scenario_id);
    row.run_id = meta.run_id;
    row.design_id = meta.design_id;
    row.replicate_id = meta.rep;
    row.attempt_used = meta.attempt;
    row.seed = meta.seed;
    row.timestamp = string(meta.timestamp);
    row.status = string(status);
    row.error_message = string(errMsg);
    row.run_validity = string(evalInfo.run_validity);
    row.missing_required_logs = string(evalInfo.missing_required_logs);
    row.missing_required_logs_count = evalInfo.missing_required_logs_count;
    ...
    row.completed_validity = string(evalInfo.metric_validity.completed);
    row.completion_time_validity = string(evalInfo.metric_validity.completion_time);
    row.average_segment_speed_validity = string(evalInfo.metric_validity.average_segment_speed);
    row.sign_detection_quality_validity = string(evalInfo.metric_validity.sign_detection_quality);
    row.tracking_error_rms_validity = string(evalInfo.metric_validity.tracking_error_rms);
    row.left_distance_to_sign_validity = string(evalInfo.metric_validity.left_distance_to_sign);
    row.right_distance_to_sign_validity = string(evalInfo.metric_validity.right_distance_to_sign);
    row.stop_distance_to_sign_validity = string(evalInfo.metric_validity.stop_distance_to_sign);
    row.time_turning_validity = string(evalInfo.metric_validity.time_turning);
    row.time_to_next_blob20_first_validity = string(evalInfo.metric_validity.time_to_next_blob20_first);
    row.time_to_next_blob20_count_validity = string(evalInfo.metric_validity.time_to_next_blob20_count);
    ...
    row.run_dir = string(runDir);
    row.logsout_file = string(logs_file);
    row.summary_json = string(json_file);
end
