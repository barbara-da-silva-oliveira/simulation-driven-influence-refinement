function runControlRegimeCampaignStructural(cfg)
...
function z = normalizeToUnitInterval(x, xMin, xMax)
...
function stats = summarizeEffectSet(compEff, rawEff, slopeEff)
    stats = struct();
    stats.MuStarComparable = mean(abs(compEff), 'omitnan');
    stats.SigmaComparable = std(compEff, 0, 'omitnan');
    stats.MuStarRawChange = mean(abs(rawEff), 'omitnan');
    stats.SigmaRawChange = std(rawEff, 0, 'omitnan');

    if nargin < 3 || isempty(slopeEff)
        stats.MuStarOriginalUnitSlope = NaN;
        stats.SigmaOriginalUnitSlope = NaN;
    else
        stats.MuStarOriginalUnitSlope = mean(abs(slopeEff), 'omitnan');
        stats.SigmaOriginalUnitSlope = std(slopeEff, 0, 'omitnan');
    end

    stats.NumEffects = numel(compEff);
end
...
function w = normalizeNonnegativeMax(mu)
    mu = double(mu(:));
    mu(mu < 0) = 0;
...
function w = normalizeNonnegativeSum(mu)
    mu = double(mu(:));
    mu(mu < 0) = 0;
    if isempty(mu) || all(mu == 0)
